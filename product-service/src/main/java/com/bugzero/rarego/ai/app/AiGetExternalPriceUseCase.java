...
public class AiGetExternalPriceUseCase {
	...
	public Flux<String> execute(AiExternalPriceRequestDto dto) {
		...
		return Flux.defer(() -> {
			...
			aiMetrics.incrementExternalRequest();
			aiMetrics.externalStart();
			...
			return chatClient.prompt()
				...
				.doOnError(TimeoutException.class, e -> {
					result.set("timeout");
					aiMetrics.incrementExternalTimeout();
				})
				.doOnError(e -> {
					if (!(e instanceof TimeoutException)) {
						result.set("error");
						aiMetrics.incrementExternalError();
					}
				})
				.doFinally(signalType -> aiMetrics.externalFinish(result.get(), System.nanoTime() - startedAt))
