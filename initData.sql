...
/* ==================================================================================
   [LOAD TEST] 부하 테스트 전용 데이터 생성 (재귀 CTE 방식)
   - 유저 1,000명 생성 (public_id: 'tester_1' ~ 'tester_1000')
   - 지갑 생성 및 잔액 10억 충전
   - 테스트용 경매 1개 생성 (ID: 9999)
   ================================================================================== */

-- 1. 테스트 유저 1000명 생성
INSERT INTO member_member (deleted, email, nickname, public_id, real_name, created_at, updated_at)
WITH RECURSIVE nums AS (
    SELECT 1 AS seq
    UNION ALL
    SELECT seq + 1 FROM nums WHERE seq < 1000
)
SELECT 0, CONCAT('tester_', seq, '@test.com'), CONCAT('tester_', seq), CONCAT('tester_', seq), '테스터', NOW(), NOW()
FROM nums;

-- 2. Replica 테이블 동기화 (auction, payment)
INSERT INTO auction_member (id, deleted, email, nickname, public_id, real_name, created_at, updated_at)
SELECT id, deleted, email, nickname, public_id, real_name, created_at, updated_at
FROM member_member
WHERE public_id LIKE 'tester_%';

INSERT INTO payment_member (id, deleted, email, nickname, public_id, real_name, created_at, updated_at)
SELECT id, deleted, email, nickname, public_id, real_name, created_at, updated_at
FROM member_member
WHERE public_id LIKE 'tester_%';
...
-- 3. 지갑 생성 (잔액 10억)
INSERT INTO payment_wallet (deleted, member_id, balance, holding_amount, created_at, updated_at)
SELECT 0, id, 1000000000, 0, NOW(), NOW()
FROM member_member
WHERE public_id LIKE 'tester_%';

-- 4. 테스트용 타겟 경매 생성 (ID: 9999, 판매자: 3번(기존 Lego Master))
INSERT INTO auction_auction (id, deleted, created_at, updated_at, product_id, seller_id, duration_days, start_price, tick_size, status, start_time, end_time, current_price, extension_count)
VALUES (9999, 0, NOW(), NOW(), 1, 3, 30, 1000, 100, 'IN_PROGRESS', NOW(), DATE_ADD(NOW(), INTERVAL 30 DAY), 1000, 0);