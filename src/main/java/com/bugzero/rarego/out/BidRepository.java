...
import domain.com.bugzero.rarego.Bid;
...
public interface BidRepository extends JpaRepository<Bid, Long> {
    ...
    /**
     * 해당 회원이 진행 중인 경매에 입찰한 내역이 있는지 확인
     */
    @Query("""
                SELECT CASE WHEN COUNT(b) > 0 THEN true ELSE false END
                FROM Bid b
                JOIN Auction a ON b.auctionId = a.id
                JOIN AuctionMember m ON b.bidderId = m.id
                WHERE m.publicId = :publicId
                AND a.status <> 'ENDED'
            """)
    boolean existsActiveBidByPublicId(@Param("publicId") String publicId);
}
